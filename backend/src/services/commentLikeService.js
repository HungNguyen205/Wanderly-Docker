...
const toggleLike = async (commentId, userId) => {
    try {
        ...
        const result = await request.execute("sp_CommentLikes_Toggle");
...
const getUsersWhoLiked = async (commentId, page = 1, limit = 50) => {
    try {
        const pool = await poolPromise;
        const request = pool.request()
            .input("CommentId", sql.Int, commentId)
        ...
        const result = await request.execute("sp_CommentLikes_GetByCommentId");
        const code = result.output.Result;
...
const checkUserLiked = async (commentId, userId) => {
    try {
        ...
        const result = await request.execute("sp_CommentLikes_CheckUserLiked");
        const code = result.output.Result;
        const isLiked = result.output.IsLiked === 1;
...
const getLikeCount = async (commentId) => {
    try {
        ...
        const result = await request.execute("sp_CommentLikes_GetCount");
        const code = result.output.Result;
